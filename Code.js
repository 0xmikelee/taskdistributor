...
function getHeaderIndex(sheetName) {
	...
	if (sheetName === 'Transcribers') {
		...
		headers = ['File', 'MP3 Source', 'Original Source', 'Audio Length', 'Speakers', 'Transcriber 1', 'Transcriber 1 Distribution Date', 'Transcriber 1 Result', 'Transcriber 1 Time Taken', 'Transcriber 1 Tool', 'Transcriber 1 Issues/Bugs', 'Transcriber 2', 'Transcriber 2 Distribution Date', 'Transcriber 2 Result', 'Transcriber 2 Time Taken', 'Transcriber 2 Tool', 'Transcriber 2 Issues/Bugs']
	}
...
function distributeTasks() {
	...
	for (var i = requiredDistributionTx1.length - 1; i >= 0; i--) {
		...
		Logger.log(requiredDistributionTx1[i])
		...
		tasksMasterSheet.getRange(requiredDistributionTx1[i].rowArrayIndex + 2, headerIndexTasksMaster['Transcriber 1 Tool'], 1, 1)
			.setFormula('=ImportRange("https://docs.google.com/spreadsheets/d/' + thisTxFileId + '", "Tasks!" & address(' + newRowIndex + ',' + headerIndexTxSheet['Tool']+ '))')
	...
	Logger.log(requiredDistributionTx2)
	for (var i = requiredDistributionTx2.length - 1; i >= 0 ; i--) {
		...
		tasksMasterSheet.getRange(requiredDistributionTx2[i].rowArrayIndex + 2, headerIndexTasksMaster['Transcriber 2 Time Taken'], 1, 1)
			.setFormula('=ImportRange("https://docs.google.com/spreadsheets/d/' + thisTxFileId + '", "Tasks!" & address(' + newRowIndex + ',' + headerIndexTxSheet['Time Taken']+ '))')
		tasksMasterSheet.getRange(requiredDistributionTx2[i].rowArrayIndex + 2, headerIndexTasksMaster['Transcriber 2 Tool'], 1, 1)
			.setFormula('=ImportRange("https://docs.google.com/spreadsheets/d/' + thisTxFileId + '", "Tasks!" & address(' + newRowIndex + ',' + headerIndexTxSheet['Tool']+ '))')
